...
public class Exam0110 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 기존에 같은 이름의 파일이 있으면 덮어쓴다. (기존 파일 사라짐)
    // - 파일 경로가 절대 경로가 아니면
    //   - 리눅스, 유닉스 : /로 시작하지 않으면,
    ...
    // 3) 출력 도구를 닫는다.
    // - OS가 관리하는 자원 중에서 한정된 개수를 갖는 자원에 대해
    //   여러 프로그램이 공유하는 경우, 사용 후 OS에 반납해야 한다.
    //   그래야 다른 프로그램이 해당 자원을 사용할 수 있다.
    //     예) 파일, 메모리, 네트워크 연결 등
    //   이런 자원을 사용하는 클래스는 항상 자원을 해제시키는 close()라는 메서드가 있다.
    //
    // => 이번 예제에서 다루는 FileOutputStream 클래스에도 close()메서드가 있다.
    //    따라서 이 객체를 사용한 후에는 close()를 호출하여 자원을 해제시켜야 한다.
    //
    // - close()를 호출하면 FileOutputStream이 작업하는 동안 사용했던
    //   버퍼(임시메모리)를 비운다.
    // - OS에 제공한 파일과의 연결을 끊는다.
    out.close();
    // 물론, JVM을 종료하면 JVM을 실행하는 동안 사용했던 모든 자원은 자동으로 해제된다.
    // 문제는 24시간 365일 멈추지 않고 실행하는 서버 프로그램인 경우
    // 사용한 자원을 즉시 해제시키지 않으면 자원 부족 문제가 발생한다.
    // 결론!
    // - 간단한 코드를 작성하더라도 close()호출을 게을리하지 말라.
...
// ## 바이너리 파일 vs 텍스트 파일
// 1) 바이너리 파일
// - 기본 텍스트 편집기(메모장, visual studio 에디터 등)로 편집할 수 없는 파일을 말한다.
// - 만약 텍스트 편집기로 변경한 후 저장하면, 파일 포맷이 달라 파일이 손상된다.
// - 예) .pdf, .ppt, .xls, .gif, .mp3, .jpg, .hwp, .mov, .avi, .exe, .lib 등
// - 바이너리 파일을 편집하려면 해당 파일 포맷을 이해하는 전용 프로그램이 필요하다.
//
// 2) 텍스트 파일
// - 기본 텍스트 편집기(메모장, visual studio 에디터 등)로 편집할 수 있는 파일을 말한다.
// - 예) .txt, .csv, .html, .js, .css, .xml, .bat, .c, .py, .php, .docx, .pptx, .xlsx 등
// - 텍스트 파일은 전용 에디터가 필요없다.
//
//
