...
// CarObserer 구현체를 만들 때
// - 인터페이스에 선언된 모든 메서드를 구현해야 한다.
// - 관심없는 메서드라도 구현해야 한다.
// 예)
//  SafebelfCarObserver는 시동 켤 때 작업을 수행한다.
//  그래서 carStarted() 메서드에 코드를 삽입하였다.
//  그런데 인터페이스를 구현하려면 모든 메서드를 정의해야하기 깨문에
//  관심이 없는데도 불구하고 carStopped() 메스드도 구현하였다.
//  물론 코드가 없는 빈 메서드이다.

// 인터페이스를 좀더 쉽게 구현하는 방법:
// - 위와 같은 경우에,
