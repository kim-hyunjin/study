...
public class App {
  ...
  // 애플리케이션의 서비스가 시작될 때 등록된 리스너에게 알린다.
  private void notifyApplicationInitialized() {
    for(ApplicationContextListener a : listeners) {
      // 옵저버를 실행할 때 작업 결과를 받을 수 있도록 바구니를 넘긴다.
      // 물론 옵저버에게 전달할 값이 있으면 넘겨주기 전에 값을 바구니에 담도록 한다.
      // 파라미터로 Map과 같은 객체를 사용하면 이런 점에서 편하다.
      // 즉 파라미터로 값을 전달하고(In) 전달받을(Out)수 있다.
      a.contextInitailized(context);
    }
  }

  // 애플리케이션의 서비스가 종료되면 등록된 리스너에게 알린다.
  ...
  public void service() {
    ...
    // 즉 DataLoaderListener를 실행.
    notifyApplicationInitialized();

    // 옵저버 실행 후 List객체를 꺼내보자.
