...
import { BarChart3, LineChart, Network, LayoutGrid, Home, CandlestickChart } from 'lucide-react';

const Sidebar: React.FC = () => {
  const menuItems = [
    { name: '대시보드 홈', path: '/', icon: <Home size={20} /> },
    { name: '거래소 차트', path: '/trading', icon: <CandlestickChart size={20} /> },
    { name: '동적 막대 차트', path: '/bar', icon: <BarChart3 size={20} /> },
    { name: '인터랙티브 라인', path: '/line', icon: <LineChart size={20} /> },
  ...
  return (
    <div className="w-64 h-screen bg-slate-900 text-white flex flex-col fixed left-0 top-0">
      <div className="p-6">
        <h1 className="text-2xl font-bold text-blue-400">D3 Showcase</h1>
      </div>
