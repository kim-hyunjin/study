...
const LineChart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const tooltip = d3.select(containerRef.current)
      .append('div')
      .attr('class', 'absolute bg-white p-2 rounded border border-gray-200 shadow-lg pointer-events-none text-xs')
      .style('opacity', 0);
    ...
    svg.append('rect')
      ...
      .on('mousemove', (event) => {
        ...
        if (containerRect) {
           ...
           const tooltipY = (y(d.value) / height) * containerRect.height;
  ...
  return (
    <div ref={containerRef} className="p-4 bg-white rounded-xl shadow-md relative">
      <h2 className="text-xl font-bold text-gray-800 mb-2">인터랙티브 라인 차트 (Interactive Line Chart)</h2>
      <p className="text-sm text-gray-600 mb-6">
        마우스 오버 시 가장 가까운 데이터 포인트를 찾아 툴팁과 가이드라인을 표시합니다.
        D3의 `bisector`를 활용한 정밀한 인터랙션 구현이 가능하여 시계열 데이터 분석에 최적입니다.
      </p>
